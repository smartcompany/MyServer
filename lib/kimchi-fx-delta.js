 ...
 * x = ((fx / fx_reference) - 1) * 100 (기준 대비 %)
 * @param {number} fx USD/KRW
 * @param {{ type?:string, fx_reference?:number, bias_pp?:number, k_pp_per_fx_percent?:number, high_fx_onset_inclusive?:number|null, k_hi_pp_per_fx_percent_squared?:number, clamp_min?:number|null, clamp_max?:number|null } | null} deltaModel
 */
function deltaAddPpForFxAffineRatio(fx, deltaModel) {
  ...
  const biasPp = coerceNumber(deltaModel.bias_pp);
  const k = coerceNumber(deltaModel.k_pp_per_fx_percent);
  if (fxReference == null || fxReference <= 0 || biasPp == null || k == null) {
    return null;
  }

  const fxNum = Number(fx);
  if (fxNum <= 0) return biasPp;

  const fxPctFromRef = (rate) => (rate / fxReference - 1.0) * 100.0;
  const x = fxPctFromRef(fxNum);
  let d = biasPp + k * x;

  const onset = coerceNumber(deltaModel.high_fx_onset_inclusive);
  const kHi = coerceNumber(deltaModel.k_hi_pp_per_fx_percent_squared) ?? 0;
  if (onset != null && onset > 0 && kHi !== 0 && fxNum >= onset) {
    const xOnset = fxPctFromRef(onset);
