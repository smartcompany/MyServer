 ...
 * - 데이터 없음(파일 없음/비어 있음) → 60일 전부터 현재까지 조회 후 생성
...
async function updateUsdKrwData() {
  ...
  try {
    ...
    usdKrw = {
      ...
      time_unit: 'hour',
      source: 'yfinance',
      note: 'Yahoo 제한으로 최대 약 60일치만 제공됩니다.',
      series: [],
    };
  }
  ...
  // 데이터 없으면 60일 전부터, 있으면 마지막 시각부터
  const startMs = usdSeries.length
    ? parseDt(usdSeries[usdSeries.length - 1].datetime)?.getTime()
    : targetEndMs - 60 * 24 * 60 * 60 * 1000;
  ...
  for (const q of quotes) {
    ...
    const dtStr = formatHourIso(date);
  ...
  const newLastMs = newLast ? parseDt(newLast.datetime)?.getTime() : 0;
  let filled = 0;
...
async function updateUsdtData() {
  ...
  if (added > 0) console.log(`[chart-update] USDT ${added}개 봉 보강/채움 → upbit_usdt_hour.json 갱신`);
}
